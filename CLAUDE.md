...
- **Backtest (`backtest.py`, overnight close-to-open gap, deployed default weights `0.8/0.0/0.2/0.0`, `±0.25` thresholds, untouched/no phase2-specific tuning)**: 79/161 trades, **total return +64.51%**, hit rate 58.2%, Sharpe/trade 1.29, max drawdown 39.52%, Correct/Flat/Wrong = 25/37/17. Beats always-long (-50.87%) and always-flat (0%).
- **Threshold sweep, rechecked at N=161** (`experiments/phase2_threshold_sweep.py`, `outputs/global/summary/phase2_threshold_sweep.tsv`) - the N=153 claim that tightening always hurts does NOT hold at N=161: `hu=0.15` (123 trades, most aggressive) beats the deployed `hu=0.25` on total return (84.29% vs 64.51%) though avg-P&L/trade is lower (0.60% vs 0.77%). The single-metric total-return-optimal point on this grid is actually `hu=0.35` - **wider** than deployed, not tighter (47 trades, total return 106.91%, avg 1.71%/trade, hit 66.0%, Sharpe 2.13, maxDD 12.63% - better than deployed on every axis except trade count). This is a single in-sample grid with no LOOCV/DSR/permutation check (unlike the weight+threshold sweep below) - directional signal only, not validated.
...
- **The blend weights/thresholds are not an overfit - reconfirmed at N=161** - `experiments/phase2_pnl_weight_threshold_sweep.py` (phase2-scoped 113,344-combo weight-simplex x asymmetric-threshold grid, total-return objective) rerun at N=161 (`outputs/global/summary/phase2_pnl_weight_threshold_sweep.json`): the in-sample "best" combo (`[micro=0.55, macro=0.45, news=0, quant=0]`, thr `0.25/-0.05`, 99 trades) shows total return 167.66% and raw Sharpe 2.329, but **PSR (Deflated Sharpe Ratio) = 0.0** (not real skill) and **permutation p=0.150** (not significant) - same failure mode as the N=153 run, now confirmed current. LOOCV total return (87.82%, 95 trades) is positive and beats the deployed default's 64.3% in this LOOCV comparison, but PSR/permutation still say the in-sample edge is trial-count luck, not real skill - **don't promote**. The deployed default weights (`0.8/0.0/0.2/0.0`, `±0.25`) remain the only defensible choice on phase2. A full paste-ready Data Entry row set for this unvalidated candidate was generated anyway on request (`phase2/export_rows_aggressive.py` -> `phase2/data_entry_rows_aggressive.tsv`, rater label `Ben/DeepSeek (aggressive: micro.55/macro.45)`, self-checks to 167.96% total return / 99 trades against the sweep JSON) and pasted into the live sheet as an experimental extra row, separate from the deployed LLM row - see Live sheet section.
...
- **`Summary`**: Accuracy-by-Rater (pre-existing, cols A:G) sits beside the new P&L-by-Rater (cols I:Q: `Rater, Type, #Trades, Total Return %, Avg %/Trade, Dir. Hit Rate, Correct, Flat, Wrong`), same 7 raters/rows for side-by-side reading, plus an All-Humans aggregate row. `#Trades` excludes HOLD (why LLM shows 79, not 161). **Avg Net P&L per Trade is the recommended primary judging metric** (fair across raters with different trade counts; Total Return/Accuracy are supporting context only). Accuracy-by-Company and P&L-by-Company are similarly paired, company names pulled by reference so the two blocks can't drift out of sync. **Known formula bug**: Anna's row and the All-Humans-Total row's Total Return % cells (`M` column) show `#VALUE!` - both use `PRODUCT(1+COM.MICROSOFT.FILTER(...))-1`; Anna's own P-column data has no bad/error values, so this looks like a FILTER-function evaluation quirk in this sheet's engine, not corrupt input - not yet root-caused.
- **Experimental extra rater row**: `Ben/DeepSeek (aggressive: micro.55/macro.45)` was pasted in as an additional Data Entry row set (99 trades, ~168% total return) alongside the deployed `Ben/DeepSeek` LLM row - see the weight+threshold sweep bullet above for why it's unvalidated (PSR=0.0, perm p=0.150). It won't appear in the Summary sheet's per-rater table automatically (that table's rater rows are static); add a row there manually if it needs to show up.
