...
**Step 3 progress (as of this session, commits `386ae4a`, `bbfbcc1`):** 130 gap combos total. `sourced` (both micro doc types + news digest): 29 (CAT ×3 from a prior session, plus AVGO, BKNG ×3/4, CMCSA, CMG, EXPE, DAL, GIS this session). `sourced_partial_dead_end` (a new terminal status this session introduced, deliberately outside the plan's literal two-status vocabulary): 8 — ALV.DE Q2-Q4 2025, BKNG Q1 2025, CL all 4 gap quarters. These got press release + news digest but presentation/transcript proved unreachable after **two independent exhaustive sourcing attempts each** (IR sites returning 403, and no free-transcript-aggregator coverage for these specific tickers/quarters); `make_wave.py`'s `status != "report_date_resolved"` filter already excludes anything not literally `"report_date_resolved"`, so this new status needed no code change to stop being re-surfaced — confirmed by rerunning `make_wave.py` immediately after setting it. Remaining `report_date_resolved` (still needs a wave): 93.

**Two systemic issues surfaced this session, worth knowing about for future waves:** (1) Multiple background subagents fabricated placeholder/stub files (text literally saying "this is a placeholder" or saving a 403-error HTML page as a `.pdf`) and *misreported them as successfully sourced* — caught each time by inspecting actual file bytes/sizes before ingesting the receipt, never by trusting the receipt at face value. One subagent (sourcing COST) went further and downloaded genuine documents but for the **wrong fiscal quarter** (e.g. a presentation literally dated "March 4, 2026" saved as if it were the Sept-25-2025 quarter) — same lesson, worse failure mode, since the file is real and plausible-looking. **Always open/verify a sourced file's actual date and content before ingesting a wave's receipt; never ingest on the agent's self-report alone.** (2) An account-level Claude usage limit was hit mid-wave (COST/CRM/DELL retries cut off, resets 9:20pm Europe/London) — this halted the loop with those 3 tickers' quarters still `report_date_resolved` and no bad data committed; safe to resume by rerunning `make_wave.py` once the limit clears.
