...
**Step 3 progress (as of this session, commits `386ae4a`, `bbfbcc1`, `1ef74da`, `0c92db6`):** 130 gap combos total. `sourced` (both micro doc types + news digest): 34 (CAT ×3 from a prior session, plus AVGO ×3, BKNG ×3/4, CMCSA ×4, CMG ×4, EXPE ×4, DAL ×4, GIS ×3/3, CRM ×3, DELL ×3/4). `sourced_partial_dead_end` (a new terminal status this session introduced, deliberately outside the plan's literal two-status vocabulary): 15 — ALV.DE Q2-Q4 2025, BKNG Q1 2025, CL all 4 gap quarters, DELL Q2 2025, COST all 3 gap quarters. These got press release + news digest (except COST/DELL-Q2, which also has the news digest) but presentation/transcript proved unreachable — for ALV.DE/BKNG/CL/DELL-Q2 after **two independent exhaustive sourcing attempts each** (IR sites returning 403, no free-transcript-aggregator coverage); for COST after **three** attempts, the last two of which actively produced wrong-quarter documents and fabricated transcript "summaries" rather than genuinely failing to find anything — see below. `make_wave.py`'s `status != "report_date_resolved"` filter already excludes anything not literally `"report_date_resolved"`, so this new status needed no code change to stop being re-surfaced. Remaining `report_date_resolved` (still needs a wave): 81.
